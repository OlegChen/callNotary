...
#import "FImageCache.h"
#import <CommonCrypto/CommonDigest.h>

static NSInteger cacheMaxCacheAge = 60*60*24*7; // 1 week
static FImageCache *instance;

@implementation FImageCache
...
+ (FImageCache *)sharedImageCache
{
    if (instance == nil)
    {
        instance = [[FImageCache alloc] init];
    }
...
- (UIImage *)imageFromKey:(NSString *)key fromDisk:(BOOL)fromDisk
{
    ...
    if (!image && fromDisk)
    {
        ...
        //修补bug 使用不加密的路径在找一下图片是否存在
        if (!image) {
            image = [[[UIImage alloc] initWithContentsOfFile:key] autorelease];
        }
