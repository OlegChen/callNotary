...
#import "ZSYPopoverListInformView.h"
...
@interface ZSYPopoverListInformView ()
...
@implementation ZSYPopoverListInformView
...
- (void)initTheInterface
{
    ...
    self.textView = [[UITextView alloc] initWithFrame:CGRectMake(0, 42.0f, xWidth, (300.0f-42.0f))]; //初始化大小并自动释放
    self.textView.textColor = [UIColor blackColor];//设置textview里面的字体颜色
    self.textView.font = [UIFont fontWithName:@"Arial" size:18.0];//设置字体名字和字体大小
    self.textView.delegate = self;//设置它的委托方法
    self.textView.backgroundColor = [UIColor whiteColor];//设置它的背景颜色
    self.textView.text = @"申请人需严格遵守《北京市方正公证处公证告知书（保全证据类）》的相关规定，应向公证机构如实说明申请公证事项的有关情况，提供真实、合法、充分的证明材料；提供的证明材料不充分的，公证机构可以要求补充。提供虚假证明材料骗取公证书，给他人造成损失的，依法承担民事责任；违反治安管理的，依法给予治安管理处罚；构成犯罪的，依法追究刑事责任。具体公证要求请咨询：\n\n公证机构：北京市方正公证处\n              （原北京市第二公证处）\n公证人员： 冯爱芳\n咨询电话：010-58073549\n网       址：www.bjgz.org.cn\n地       址：北京西城区阜外大街7号国投大厦四层\n";
...
- (void)setCancelButtonTitle:(NSString *)aTitle block:(ZSYPopoverListViewButtonBlock)block
{
    if (nil == _cancelButton)
    {
        ...
        [self.cancelButton setBackgroundImage:[ZSYPopoverListInformView cancelButtonBackgroundImage] forState:UIControlStateNormal];
...
- (void)setDoneButtonWithTitle:(NSString *)aTitle block:(ZSYPopoverListViewButtonBlock)block
{
    if (nil == _doneButton)
    {
        ...
        [self.doneButton setBackgroundImage:[ZSYPopoverListInformView normalButtonBackgroundImage] forState:UIControlStateNormal];
