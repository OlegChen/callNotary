...
@interface ChooseChildFolder ()

@end

@implementation ChooseChildFolder
...
- (void)folderPushBtnClick:(UIButton *)btn
{
    ...
    ChooseChildFolder *child = [[ChooseChildFolder alloc] init];
    child.folderID = folderID;
    child.folderName = folderName;
    child.file = self.file;
    child.childFileModel = model;
    child.controler = self.controler;
...
{
    FolderModel *model = [self.contentArray objectAtIndex:indexPath.row];
    self.controler.folder = model;
    self.controler.model.folderId = model.folderID;
