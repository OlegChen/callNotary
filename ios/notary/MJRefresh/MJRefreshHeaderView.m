...
- (void)updateTimeLabel
{
    ...
    if ([cmp1 day] == [cmp2 day]) { // 今天
        ...
        self.lastUpdateTimeLabel.text = [NSString stringWithFormat:@"%@:%@ %@",@"最后更新",@"今天", [formatter stringFromDate:self.lastUpdateTime]];
    } else if ([cmp1 year] == [cmp2 year]) { // 今年
        formatter.dateFormat = @"MM-dd HH:mm";
        self.lastUpdateTimeLabel.text = [NSString stringWithFormat:@"%@:%@",@"最后更新", [formatter stringFromDate:self.lastUpdateTime]];
    } else {
...
//    
//    // 3.显示日期
//    self.lastUpdateTimeLabel.text = [NSString stringWithFormat:@"%@:%@",[LYUtil setLanguage:@"最后更新"], time];
    NSLog(@"%@",self.lastUpdateTimeLabel.text);
}
